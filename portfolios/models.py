...
class Portfolio(models.Model):
    ...
    def name(self):
        """
        @returns: The name of the game that this portfolio belongs to.
        """
        return self.game.game_name

    def update(self):
        """
        Forces the portfolio to recalculate it's winnings.
        """
        if not self.active:
            return
    ...
    def get_username(self):
        """
        @returns: the username or botname of the owner of this portfolio
        """
    ...
    def get_url(self):
        """
        @returns: the URL for the owner of this portfolio (AI or Human)
        """
        if self.ai_opponent:
            return "/agent/%s" % self.agent.name
        
        return "/user/%s" % self.user_profile.user.username

    def get_winnings(self):
        """
        @returns: The current winnings of this portfolio
        """
        return self.get_equity() - self.game.initial_funds 

    def get_equity(self):
        """
        @returns: The current total value of this portfolio
        """
        equity = self.cash
        owned_stocks = self.ownedstock_set.all()
        for owned in owned_stocks:
            stock = Stock.objects.get(ticker=owned.ticker)
            equity += stock.current_price * owned.quantity

        return equity
    
    def get_equity_quick(self):
        return self.cash + self.winnings + self.game.initial_funds
    
    def get_transaction_cost(self):
        """
        @returns: The cost of every transaction for this portfolio
        """
    ...
    def get_brief_stocks(self):
        ...
        fin = []
        owned_stocks = self.ownedstock_set.filter(quantity__gt=0).order_by('ticker')
        for owned in owned_stocks:
            stock = Stock.objects.get(ticker=owned.ticker)
            fin.append((owned, stock))
        return fin
    ...
    def get_brief_watched(self):
        ...
        """
        watched = self.ownedstock_set.filter(watch=True, quantity=0)
        fin = []
        for watched in watched:
            stock = Stock.objects.get(ticker=watched.ticker)
            fin.append((watched, stock))
        return fin
    ...
    def create_order(self, stock_ticker, price, transaction, trade='sell', quantity=0):
        """
        Creates a stop loss or limit order.
        
        @param stock_ticker: The ticker of the stock to place the order against
        @param price: The price at which the order should be triggered
        @param transaction: The type of order ('STOP_LOSS', or 'LIMIT')
        @param trade: Wether to 'sell' or 'buy' for limit orders
        @param quantity: The quantity to trade for limit orders
        @returns: True if the order was places, False otherwise
        """
    ...
    def purchase_stock(self, stock_ticker, quantity):
        ...
        @returns: True if the purchase is successful, else False
        """
        # Error checking
